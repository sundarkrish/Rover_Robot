...
int circ_buff_size(circ_buff_t *ptr)
{
    if(!circ_buff_empty(ptr))                               //if the buffer has data
    {
        ...
        {
            return (ptr->head - ptr->tail);                 //simply take the difference between head and tail
        }
        else
        {
            return (ptr->size + ptr->head - ptr->tail);     //size is difference between head and tail + size.
        }
    }
